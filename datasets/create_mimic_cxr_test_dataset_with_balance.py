...
def generate_csv(metadata_path, records_path, studies_path, labels_path, disease, seed, n_samples, batch_name, use_processed, output_csv):
    ...
    # Extract balanced version for Atelectasis
    final_df_0 = final_df.query('label == 0')
    final_df_1 = final_df.query('label == 1')
    final_df_ = pd.concat([final_df_0, final_df_1.sample(244)])
    print(final_df_.label.value_counts())
    final_df_.to_csv('test_atelectasis_300_balanced.csv', index=False)
